...
export class PubSubJobQueueStrategy
  ...
  implements JobQueueStrategy
{
  ...
  async add<Data extends JobData<Data> = {}>(
    ...
    const id = await this.topic(job.queueName).publish(
  ...
  async start<Data extends JobData<Data> = {}>(
    ...
    Logger.debug(`Retrieved subscription: ${subscription}`, loggerCtx);
    const listener = (message: Message) => {
      Logger.debug(
        `Received message (listened): ${queueName}: ${message.id}`,
        loggerCtx
      );
